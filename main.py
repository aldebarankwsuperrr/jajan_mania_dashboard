...
def cleaned_vendor():
    tabel_vendor = read_data("https://docs.google.com/spreadsheets/d/197Z0h89qd2R3Z3m5aWvG4LMRBHokA6DdhjqdUHZdEzk/export?gid=1503184305&format=csv")
    tabel_vendor = tabel_vendor.drop(['address', 'email', 'password', 'username','balance','gender','exp','last_latitude','last_longitude','jajan_image','jajan_name', 'jajan_desc','status','updated_at', 'created_at','deleted_at'], axis = 1)
...
def vendor_from_forcast(similarity_vector, jajan_items, weather, ascending):
    ...
    similarity_forcast_merged_cleaned = similarity_forcast_merged.merge(tabel_vendor, left_on='vendor_id', right_on='vendor_id').dropna()
    vendor = similarity_forcast_merged_cleaned['fullname'].sort_values(ascending=ascending).unique()
...
ket_vendor_tinggi = high_transaction_vendor.merge(tabel_vendor, left_on='vendor_id', right_on='vendor_id')
ket_vendor_rendah = low_transaction_vendor.merge(tabel_vendor, left_on='vendor_id', right_on='vendor_id')
...
with Time_series:
    ...
    with col_disp: 
        ...
        ax.tick_params(axis='y', labelsize=12)                                                              #15 nov 2023
        ax.tick_params(axis='x', labelsize=12)                                                             #15 nov 2023
        ax.set_ylabel('Penjualan (Ratus Ribu Rupiah)')                                                                                 #15 nov 2023
        ax.set_xlabel('Tanggal')
        labels, locations = plt.yticks()
        plt.yticks(labels, (labels/1000).astype(int))                                                                                 #15 nov 2023
        st.pyplot(fig)  
    with col_explain:
        st.markdown("                   ")
        st.markdown("                   ")
    ...
    with col_disp:
        ...
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C001", label ='es')
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C002", label ='gorengan')
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C003", label ='jajan ringan')
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C004", label ='minum hangat')
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C005", label ='makan berat')
        sns.lineplot(data=Pembelian_per_kategori, x="created_at", y="C006", label ='Buah')                                 #15 nov 2023
        ax.set_title("Penjualan per Kategori", loc="center", fontsize=17)
        ax.legend()                                   #15 nov 2023
        ax.tick_params(axis='y', labelsize=12)                                                              #15 nov 2023
        ax.tick_params(axis='x', labelsize=12)                                                              #15 nov 2023
        ax.set_ylabel('Penjualan (Ratus Ribu Rupiah)')                                                                                 #15 nov 2023
        ax.set_xlabel('Tanggal')
...
with col_disp : 
    ...
    ax.tick_params(axis='y', labelsize=12)                                                              #15 nov 2023
    ax.tick_params(axis='x', labelsize=12)                                                              #15 nov 2023
    ax.set_ylabel('Penjualan (Rupiah)')                                                                                 #15 nov 2023
    ax.set_xlabel('Vendor ID')                                                                               #15 nov 2023
    st.pyplot(fig) 
...
with col5:                           #1 nov 2023                                             #1 nov 2023
    ...
    if user_id:                                                                                             #1 nov 2023
        recommendation2 = calculate_category_sum(user_id, transactions).merge(categories,left_on=['category_id'], right_on=['category_id']).drop(['category_id','tags'], axis = 1)
        st.write(recommendation2)
with col6:
    pembelian_user = calculate_category_sum(user_id, transactions).merge(categories,left_on=['category_id'], right_on=['category_id']).drop(['category_id','tags'], axis = 1)                                   #7 nov 2023
    fig, ax = plt.subplots(figsize=(7, 6))                                                              #7 nov 2023
    plt.pie(data=pembelian_user,x='amount', labels='category_name',autopct='%.0f%%')                            #7 nov 2023
...
forecast_bawah=jajan_items.merge(tabel_per_cuaca,left_on='category_id', right_on='category_id').sort_values(by=cuaca_besok,ascending=True).head(20)
tabel_forecast_bawah= forecast_bawah.merge(tabel_vendor, left_on='vendor_id', right_on='vendor_id')
...
with col_tinggi :
    ...
    st.table(data=tabel_forecast_atas['fullname'].unique())
with col_rendah:
    st.markdown("here are the vendors that will have their **sales decrease** based on the forecast above!")
    st.table(data=tabel_forecast_bawah['fullname'].unique())
with st.sidebar:
